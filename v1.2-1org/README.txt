Hyperledger fabric  多机部署实战（基于Fabric v1.2）

 官方fabric的更新速度还是相对较快的，v1.2跟v1.1还是有一些差异的，配置文件做了相应改动。
...
1、证书相关文件
	...
	cryptogen 可以指定配置文件：crypto-config.yaml，这个文件定义了有多少个orderer和组织，每个组织有多少个peer；
...
3、帮助脚本类文件
	...
	./generateArtifacts.sh yourchannel 其中yourchannel是通道名称；
		|- cryptogen  generate --config=./crypto-config.yaml 创建证书命令
		|- configtxgen -profile OneOrgsOrdererGenesis2 -outputBlock ./channel-artifacts/genesis.block 创建创世区块
		|- configtxgen -profile OneOrgsChannel2 -outputCreateChannelTx ./channel-artifacts/channel.tx -channelID yourchannel 创建创世区块
		|- configtxgen -profile OneOrgsChannel2 -outputAnchorPeersUpdate ./channel-artifacts/Org1MSPanchors.tx -channelID $CHANNEL_NAME -asOrg Org1MSP 创建创世区块
