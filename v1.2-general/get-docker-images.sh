...
printHelp() {
  echo "Usage: download.sh [-d -b]"
  echo
  echo "-d - 不下载docker镜像"
  echo "-b - 不下载二进制文件"
  echo
  echo "e.g. ./download.sh"
  echo "would download docker images and binaries for version 1.2.1 (fabric) 1.2.1 (fabric-ca) 0.4.10 (thirdparty)"
}

dockerFabricPull() {
  local FABRIC_TAG=$VERSION
...
dockerThirdPartyImagesPull() {
  local THIRDPARTY_TAG=$THIRDPARTY_IMAGE_VERSION
